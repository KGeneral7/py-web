...
def move_circle(
    ...
    if key == "Up":
        ...
        canvas.move(rect, 0, -10)  # 向上移動圓形，x方向不變，y方向減少10像素
    elif key == "s":
        canvas.move(rect, 0, 10)  # 向下移動圓形，x方向不變，y方向增加10像素
    elif key == "a":
        canvas.move(rect, -10, 0)  # 向左移動圓形，x方向減少10像素，y方向不變
    elif key == "d":
        canvas.move(rect, 10, 0)  # 向右移動圓形，x方向增加10像素，y方向不變
...
"""
quit_button = Button(
    window, text="Quit", command=window.quit
)  # 建立一個按鈕，當按下按鈕時，執行window.quit()函數，關閉視窗
"""
quit_button = Button(window, text="Quit", command=exit_fun)
quit_button.pack()  # 將按鈕放置在視窗中
#######################建立畫布########################
# 設定視窗圖片
